...
app.put('/api/candidate/:id', (req, res) => {
    const errors = inputCheck(req.body, 'party_id');  // forces any PUT request to include a party_id property
    if (errors) {
        res.status(400).json({ error: errors });
        return;
    }
    ...
    WHERE id = ?`;
    const params = [req.body.party_id, req.params.id]; // affected row's id is always part of route (req.params.id), but
    // fields being updated should be party of the body (req.body.party_id)
    db.query(sql, params, (err, result) => {
        if (err) {
            res.status(400).json({ error: err.message });
